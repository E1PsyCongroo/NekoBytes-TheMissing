...
/*
  如果actual等于true则返回true,
  否则返回"Assertion error: expected (msg) to be true, but got false"。
*/
...
/*
  如果actual等于false则返回true,
  否则返回"Assertion error: expected (msg) to be false, but got true"。
*/
...
/*
  (参数类型为char)
  如果expected等于actual则返回true,
  否则返回:"Assertion error: expected (msg) to be (expected) but got (actual)"
*/
...
// 注意：这个函数仅适用于被create_default_state所创建的state)
bool assert_state_equals(game_state_t* expected, game_state_t* actual) {
  ...
  // 检查蛇的坐标以及存活状态是否一样
  // 注意：这个代码会检查所有蛇，如果state->snakes没有被正确分配可能会报错
